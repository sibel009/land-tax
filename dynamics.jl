...
function transition_interp_loss!(loss, xx, τ_path, t, params)
    ...
    (; ℵ, n, ω, ϱ, σ, β, A, δ_k, δ_s, 
        α_C, α_K, a_L, cost_L, Lbar, ρ_h, ρ_H, R_R, nvars) = params
    ...
    p_LDV0 = (1 + τ_LDV_sale0)*p_LR0 + cost_L
    ...
    tax_wedge_S = 1 + τ_S_inv0*(1+n) + d_S0*(1+τ_D_S0)
    tax_wedge_s = 1 + τ_s_inv0*(1+n) + d_s0*(1+τ_D_s0)
    tax_wedge_K = 1 + τ_K_inv0*(1+n) + d_K0*(1+τ_D_K0)
    
    LM_L0   = LM_C0*(1+n)*((1+τ_LDV_sale0)*p_LR0 + cost_L) - β*LM_C1*((1+τ_LDV_sale1)*p_LR1 + cost_L)
    LM_R_L0 = LM_R_C0*(1+n)*(1-τ_LR_sale0)*p_LR0 - β*LM_R_C1*(1-τ_LR_sale1)*p_LR1


    # -------- government revenue ----------
    T_K_inv0    = τ_K_inv0*((1+n)*K1 - (1-δ_k)*K0)
    T_K0        = τ_K0*R_K_gross0
    T_Ss_inv0   = τ_S_inv0*((1+n)*S1 - (1-δ_s)*S0) + τ_s_inv0*((1+n)*s1 - (1-δ_s)*s0)
    ...
    # Euler on S
    loss[nvars*(t-1)+3] = (marg_benef_H*dH_dS1 + β*LM_C1*(1-δ_s)*(1+τ_S_inv1) - LM_C0*tax_wedge_S) # ^2
    
    # Euler on s
    loss[nvars*(t-1)+4] = (marg_benef_h*dh_ds1 + β*LM_C1*(1-δ_s)*(1+τ_s_inv1) - LM_C0*tax_wedge_s) # ^2
    
    # Euler on K
    loss[nvars*(t-1)+5] = (β*LM_C1*(R_K_net1 + (1-δ_k)*τ_K_inv1) - LM_C0*tax_wedge_K) # ^2
...
function unpack_sol!(sol, Res, init_cond, terminal_cond, τ_path, params, T0)
    ...
    sol_path = [init_cond; sol; terminal_cond]

    K_path = zeros(T_loc+2)
    S_path = zeros(T_loc+2)
    s_path = zeros(T_loc+2)
    L_path = zeros(T_loc+2)
    l_path = zeros(T_loc+2)
    c_path = zeros(T_loc+2)
    C_path = zeros(T_loc+2)
    C_R_path  = zeros(T_loc+2)
    p_LR_path = zeros(T_loc+2)
    ...
    p_LDV_path       = @. (1 + τ_path[:, nτ_LDV_sale]) * p_LR_path + cost_L
...
τ_path_PostT1_exprmt2[:, nτ_K_inv]  .= 0.02
...
# Experiment 12: A transient (one-period) increase in capital tax to 2%
τ_path_PostT1_exprmt12   = copy(τ_path_PostT1_eq0)
τ_path_exprmt12          = copy(τ_path_eq0)

τ_path_PostT1_exprmt12[:, nτ_K_inv]  .= [0.02; zeros(T-T1+2-1)]
...
for k in 13:13
    ...
    init_cond     = SVector{5}(init_ss[1:5])
    terminal_cond = SVector{4 + 9}([terminal_ss[6:9]; terminal_ss])

    # create result matrices
    Res_PF           = @MMatrix zeros(T+2, 9 + 21 + 1 + 30)
    Res_PF[:, 1:9]   = repeat(init_ss', T+2)
    Res_PF[:, 10:31] = repeat(init_ss_derived', T+2)
    Res_PF[T1+1:end, 32:end] = τ_path_PostT1
